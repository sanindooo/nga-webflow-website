...
${rows}`;
  ...
  // src/utils/footerTextSticky.ts
  var footerTextSticky = () => {
    const section = document.querySelector(".section_follow-banner");
    if (!section) return;
    const component = section.querySelector(".footer_follow-component");
    if (!component) return;
  ...
  // src/utils/heroBackgroundVideo.ts
  ...
  var buildEmbedUrl = (source) => {
    if (source.platform === "youtube") {
      ...
      return `https://www.youtube-nocookie.com/embed/${source.id}?${params2}`;
    }
  ...
  var allowAttrFor = (platform) => platform === "youtube" ? "autoplay; encrypted-media" : "autoplay; fullscreen";
  var markReady = (container) => container.classList.add("is-video-ready");
  var watchYouTubePlayback = (iframe, onPlaying) => {
  ...
  var watchVimeoPlayback = (iframe, onPlaying) => {
    const subscribe = () => iframe.contentWindow?.postMessage(
  ...
  var heroBackgroundVideo = () => {
    const containers = document.querySelectorAll("[data-hero-video]");
  ...
  // src/utils/projectInfoButton.ts
  var projectInfoButton = () => {
    const button = document.querySelector(".hero-open_modal");
    if (!button) return;
    ...
    const figures = document.querySelectorAll("figure.dynamic-image_item");
    if (!figures.length) return;
    const activeFigures = /* @__PURE__ */ new Set();
    let evaluatePending = false;
    const getButtonCenter = () => {
      const rect = button.getBoundingClientRect();
      return rect.top + rect.height / 2;
    };
    const coversRight = (figure) => {
